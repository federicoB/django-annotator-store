...
def user_passes_test_ajax_403(test_func, login_url=None,
    ...
    :param test_func: function to test user is authorized
    :param login_url: page to redirect is user is not authenticated
    :param redirect_field_name: the URI query field name with the URI of the page to redirect to after login
    '''
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            # (redirect to login also not useful for ajax)
                ...
                # the following code is default user_passes_test logic from django
                # get the current request full path
                path = request.build_absolute_uri()
                # get the URL of the login page
                resolved_login_url = resolve_url(login_url or settings.LOGIN_URL)
                # If the login url is the same scheme and net location then just
                # use the path as the "next" page after login. This is just an optimization.
                login_scheme, login_netloc = urlparse(resolved_login_url)[:2]
                current_scheme, current_netloc = urlparse(path)[:2]
                # if the login scheme in empty or is the same as current
                if ((not login_scheme or login_scheme == current_scheme) and
                        # if the login host is empty or is the same as current
...
def permission_required(perm, login_url=None):
    ...
    :param perm: permission list in django format
    :param login_url: url of the login page to redirect if permission test fail
    '''
